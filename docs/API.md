...
Jukkit 框架提供了较为完整的 TypeScript 类型定义，现代编辑器（如 VS Code）通常可自动识别并启动代码补全和类型检查。
...
- [资源访问](#资源访问)
- [工具方法](#工具方法)
- [TypeScript 类型支持](#typescript-类型支持)
...
| 引入方式               | 解析位置                                |
| ---------------------- | --------------------------------------- |
| `require('Logger')`    | 内部模块（`.jukkit/modules/Logger.js`） |
| `require('./utils')`   | `src/utils.js`                          |
| `require('../config')` | 上级目录                                |
...
### jukkit.addEventListener(eventName, handler)

监听指定事件（支持类型推断）。`jukkit.on` 是此方法的简写形式。
...
### jukkit.addEventListener(eventName, priority, handler)

带优先级监听事件。

```javascript
// 优先级："LOWEST", "LOW", "NORMAL", "HIGH", "HIGHEST", "MONITOR"
jukkit.addEventListener("BlockBreakEvent", "HIGH", function(event) {
...
### jukkit.registerCommand(name, executor)

简单命令注册。

```javascript
jukkit.registerCommand("ping", function(sender, cmd, label, args) {
    sender.sendMessage("Pong!");
    return true;
});
```

### jukkit.registerCommand(name, executor, tabCompleter)

带 Tab 补全的命令注册。

```javascript
jukkit.registerCommand("hello", function(sender, cmd, label, args) {
...
### jukkit.registerCommand(name, options)
...
jukkit.registerCommand("hello", new CommandOptions()
...
```javascript
// 获取插件数据目录路径
const dataPath = jukkit.getDataDirPath();  // 返回 "plugins/插件名"
```
...
```javascript
// 获取当前运行平台类型
const platformType = jukkit.platform.type;  // "BUKKIT"

// 获取 Minecraft 版本号
const mcVersion = jukkit.platform.mcVersion;  // "1.20.4"
